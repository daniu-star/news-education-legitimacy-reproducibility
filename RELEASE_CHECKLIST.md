...
- [ ] Run `src/reproducible/final_analysis/run_formal_gee_models.py` on the controlled private data root and replace manuscript-facing aggregate coefficient tables with its outputs.
- [ ] Run `src/reproducible/recompute_fastopic_dominant_shares.py` with the frozen seed and compare its counts against the archived audit table before claiming exact reproduction.
