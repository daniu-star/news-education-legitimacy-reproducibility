...
OUT=BASE/'stage2b_review_high_tranche01'
...
urgent=(queue[queue['review_priority_stage2b'].eq('高')].copy().sort_values(['review_score','reply_comment_id'],ascending=[False,True]).head(500).reset_index(drop=True))
assert len(urgent)==500, len(urgent)
...
out.to_csv(OUT/'high_tranche01_500_agent_adjudicated_round1.csv',index=False,encoding='utf-8-sig')
...
full.to_csv(OUT/'high_tranche01_500_agent_adjudicated_full.csv.gz',index=False,compression='gzip',encoding='utf-8')
...
blind[blind_cols].to_csv(OUT/'high_tranche01_500_secondpass_blind_input.csv',index=False,encoding='utf-8-sig')
key.to_csv(OUT/'high_tranche01_500_secondpass_round1_key.csv',index=False,encoding='utf-8-sig')
...
with open(OUT/'high_tranche01_500_round1_summary.json','w',encoding='utf-8') as f: json.dump(summary,f,ensure_ascii=False,indent=2)
